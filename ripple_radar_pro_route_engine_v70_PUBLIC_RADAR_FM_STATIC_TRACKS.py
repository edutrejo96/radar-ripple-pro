...
BUILD_ID = "v70_2026_05_11_PUBLIC_RADAR_FM_STATIC_TRACKS"
BUILD_NOTE = "Build único: Radar FM público por static/musica + tracks renombrados · fichas A→B con pruebas fijas"
...
def inject_music_player() -> None:
    """Radar FM público para Streamlit Cloud.

    Versión v70:
    - Busca música en `static/musica/`.
    - Sirve los audios usando Static Serving de Streamlit: `/app/static/musica/...`.
    - Evita el servidor local 127.0.0.1, que en una web pública apunta al PC del visitante
      y no al servidor de Streamlit Cloud.
    - Mantiene estado de canción/tiempo/play en localStorage para reducir cortes por rerun.
    """
    try:
        import os as _os
        ...
        music_dir = _os.path.join(base_dir, "static", "musica")
            ...
            for root, _, files in _os.walk(music_dir):
                for fn in files:
                    if fn.lower().endswith(exts):
                        ...
                        tracks.append({"title": title, "url": f"/app/static/musica/{rel_url}"})
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">static/musica</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if not tracks:
            st.info("No hay canciones en `static/musica/`. Sube MP3 renombrados como track_01.mp3, track_02.mp3... y reinicia la app.")
            st.markdown("</div>", unsafe_allow_html=True)
            return

        playlist_json = _json.dumps(tracks, ensure_ascii=False)
        count = len(tracks)
        player_html = f"""
        <div id="rrp-fm-wrap" style="width:100%;font-family:Inter,system-ui,Segoe UI,sans-serif;color:#E2E8F0;margin-top:.55rem;">
          ...
          <audio id="rrpAudio" controls preload="metadata" style="width:100%;"></audio>
        ...
        (function() {{
          ...
          const KEY_IDX = 'rrpRadarFmIndexV70';
          const KEY_RANDOM = 'rrpRadarFmRandomModeV70';
          const KEY_PLAYING = 'rrpRadarFmPlayingV70';
          function keyTime(i) {{ return 'rrpRadarFmTimeV70_' + String(i); }}
          ...
          function loadTrack(i, shouldPlay) {{
            ...
            if (audio.src !== new URL(t.url, window.location.origin).href) {{
              audio.src = t.url;
              audio.load();
          ...
          function prevTrack(shouldPlay) {{ loadTrack(idx - 1, shouldPlay); }}
          ...
          btnRandom.addEventListener('click', () => {{ randomMode = !randomMode; paintRandom(); save(); if (randomMode) nextTrack(true); }});
          select.addEventListener('change', () => {{ idx = parseInt(select.value || '0', 10); loadTrack(idx, false); }});
        ...
        st.warning(f"No se pudo cargar Radar FM público: {exc}")
    return
