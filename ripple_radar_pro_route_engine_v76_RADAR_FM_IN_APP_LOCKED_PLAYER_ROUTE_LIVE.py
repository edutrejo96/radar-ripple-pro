...
BUILD_ID = "v76_2026_05_11_RADAR_FM_IN_APP_LOCKED_PLAYER_ROUTE_LIVE"
BUILD_NOTE = "Radar FM interno con desbloqueo de audio + auto-next + fichas A→B vivas"
...
def inject_music_player() -> None:
    ...
    v76 elimina el modo de pestaña externa. Todo vive dentro de la app.
    La única limitación real es del navegador: el usuario debe pulsar una vez
    dentro del reproductor para desbloquear audio. Después, el mismo elemento
    <audio> queda autorizado y puede hacer siguiente/anterior/auto-next sin
    recargar Streamlit.
    """
    try:
        import os as _os
            ...
            if exists and found and music_dir is None:
                ...
                for path in found:
                    ...
                    tracks.append({
                        ...
                        "fallback_url": f"static/musica/{rel_url}",
                        "file": rel_fs,
                    })
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">interno · auto-next · v76</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
...
<body>
  <div class="box">
    ...
    <audio id="audio" controls preload="auto" playsinline controlsList="nodownload"></audio>
    ...
    <div class="status" id="status">Pulsa el botón azul una vez. Después el cambio de canción y el final continúan dentro de la app.</div>
    <div class="small">Diagnóstico: <a id="staticLink" href="#" target="_blank">abrir archivo actual</a></div>
  </div>
<script>
(() => {{
  const tracks = {tracks_json};
  const KEY_IDX = "rrp_fm_v76_idx";
  const KEY_RANDOM = "rrp_fm_v76_random";
  const KEY_VOLUME = "rrp_fm_v76_volume";
  ...
  function setSource(useFallback=false) {{
    triedFallback = useFallback;
    const t = current();
    audio.src = useFallback ? (t.fallback_url || t.url) : t.url;
    audio.load();
  ...
  async function safePlay(label="Reproduciendo.") {{
    try {{
      ...
      setStatus("Bloqueo del navegador: pulsa el botón azul o el play nativo del audio una vez. No es fallo del radar ni de los MP3.", "warn");
      return false;
    }}
  }}
  ...
  unlockBtn.addEventListener("click", async () => {{ setSource(false); await safePlay("Audio desbloqueado. Radar FM queda dentro de la app."); }});
  ...
  picker.addEventListener("change", async () => {{ idx = parseInt(picker.value, 10) || 0; setSource(false); await safePlay("Canción seleccionada."); }});
  audio.addEventListener("ended", () => goNext(true));
  audio.addEventListener("play", () => {{ unlocked = true; playBtn.textContent = "⏸️ Pausa"; unlockBtn.textContent = "✅ Audio desbloqueado dentro de la app"; }});
  audio.addEventListener("pause", () => {{ if (!audio.ended) playBtn.textContent = "▶️ Play"; }});
  audio.addEventListener("volumechange", save);
  audio.addEventListener("error", async () => {{
    if (!triedFallback && current().fallback_url) {{
      setStatus("Probando ruta alternativa del archivo...", "warn");
      setSource(true);
      if (unlocked) await safePlay("Ruta alternativa cargada.");
    }} else {{
      setStatus("No se pudo cargar este track. Revisa que exista en static/musica y que GitHub lo haya subido completo.", "bad");
    }}
  }});

  updateRandom();
  setSource(false);
  setStatus("Pulsa el botón azul una vez. Luego Siguiente y fin de canción continúan dentro de la app.");
}})();
</script>
</body>
</html>
"""
        _st_components.html(player_html, height=360, scrolling=False)
        with st.expander("🔎 Diagnóstico Radar FM", expanded=False):
            st.write("Modo:", "HTML audio component v76 interno, sin pestaña externa")
