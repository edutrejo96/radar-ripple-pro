...
BUILD_NOTE = "Radar FM interno robusto v77: rutas múltiples + AudioContext + fallback st.audio + fichas A→B vivas"
...
def inject_music_player() -> None:
    ...
    v77 usa un reproductor HTML interno más robusto:
    - crea/rehabilita AudioContext desde un gesto real del usuario;
    - prueba varias rutas estáticas posibles de Streamlit Cloud;
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">interno · auto-next · v77</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
...
(() => {
  ...
  const KEY_IDX = "rrp_fm_v77_idx";
  const KEY_RANDOM = "rrp_fm_v77_random";
  const KEY_VOLUME = "rrp_fm_v77_volume";
  ...
  function tryNextCandidate() {
    ...
    if (candidateIndex >= candidates.length) {
      setStatus("No se pudo cargar este track. Abre el diagnóstico: si el enlace no abre el MP3, la ruta está mal o el archivo no está en GitHub.", "bad");
      return false;
    }
    audio.src = candidates[candidateIndex];
    audio.load();
    staticLink.href = candidates[candidateIndex];
    setStatus("Probando ruta alternativa " + (candidateIndex + 1) + "/" + candidates.length + "...", "warn");
    return true;
  }
  function nextIndex() {
  ...
  async function safePlay(label="Reproduciendo.") {
    try {
      ...
      setStatus("El navegador ha bloqueado el play programático (" + (e && e.name ? e.name : "error") + "). Pulsa el ▶ nativo del reproductor de arriba una vez; después Siguiente y auto-next deberían seguir.", "warn");
      return false;
    }
  }
  async function goNext(auto=false) {
    ...
    idx = nextIndex();
    setSource();
    if (auto || wasPlaying || unlocked) await safePlay("Siguiente reproduciendo automáticamente.");
  }
  async function goPrev(auto=false) {
    const wasPlaying = !audio.paused && !audio.ended;
    idx = (idx - 1 + tracks.length) % tracks.length;
    setSource();
    if (auto || wasPlaying || unlocked) await safePlay("Anterior reproduciendo automáticamente.");
  }

  unlockBtn.addEventListener("click", async () => { await unlockContext(); setSource(); await safePlay("Audio desbloqueado. Radar FM queda dentro de la app."); });
  ...
  picker.addEventListener("change", async () => { idx = parseInt(picker.value, 10) || 0; setSource(); if (unlocked) await safePlay("Canción seleccionada."); });
  ...
  audio.addEventListener("error", async () => {
    if (tryNextCandidate()) {
      if (unlocked) await safePlay("Ruta alternativa cargada.");
    }
  });
...
'''.replace('__TRACKS_JSON__', tracks_json)
        ...
        with st.expander("🛟 Modo seguro si el componente HTML queda bloqueado", expanded=False):
            ...
            selected = st.selectbox("Canción modo seguro", titles, index=0, key="rrp_fm_safe_track_v77")
        ...
        with st.expander("🔎 Diagnóstico Radar FM", expanded=False):
            st.write("Modo:", "HTML audio component v77 interno + fallback st.audio")
