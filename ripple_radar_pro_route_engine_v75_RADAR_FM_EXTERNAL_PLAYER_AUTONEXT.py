...
BUILD_ID = "v75_2026_05_11_RADAR_FM_EXTERNAL_PLAYER_AUTONEXT"
BUILD_NOTE = "Radar FM externo con auto-next real + fichas A→B vivas"
...
def inject_music_player() -> None:
    """Radar FM web para Streamlit Cloud.

    v74 cambia st.audio() por un reproductor HTML interno con JavaScript.
    Motivo: st.audio() es robusto, pero no permite reproducir automáticamente al pasar
    a la siguiente canción. El navegador sí permite continuidad dentro del mismo elemento
    <audio> después de que el usuario pulse Play una vez.

    Reglas:
    - primer Play: lo pulsa el usuario (restricción normal del navegador);
    - botón Siguiente/Anterior: reproduce automáticamente si ya estaba sonando;
    - fin de canción: pasa a la siguiente y reproduce automáticamente;
    - no depende de player.html externo.
    """
    try:
        import os as _os
        ...
        for cand in candidate_dirs:
            ...
            if exists and found and music_dir is None:
                ...
                for path in found:
                    ...
                    tracks.append({
                        "title": title,
                        "url": f"/app/static/musica/{rel_url}",
                        "file": rel_fs,
                    })
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">auto-next externo · static/musica · v75</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        # En Streamlit Cloud, algunos navegadores bloquean el autoplay dentro del iframe del componente.
        # Por eso añadimos una salida robusta: abrir el reproductor como página estática superior.
        st.markdown(
            """
            <div style="margin:.25rem 0 .75rem 0;padding:.65rem .75rem;border:1px solid rgba(34,211,238,.25);border-radius:14px;background:rgba(15,23,42,.65);">
              <div style="font-size:.88rem;color:#E5E7EB;margin-bottom:.35rem;">Si el navegador bloquea el autoplay integrado, usa el modo externo:</div>
              <a href="/app/static/player.html" target="_blank" style="display:inline-block;background:#0891b2;color:white;text-decoration:none;border-radius:999px;padding:.48rem .85rem;font-weight:900;">🎧 Abrir Radar FM en pestaña externa</a>
              <span style="font-size:.78rem;color:#94A3B8;margin-left:.55rem;">En esa pestaña: Play una vez → siguiente y fin de canción continúan solos.</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        # HTML/JS interno: no usa Streamlit buttons, por eso no hay rerun ni corte de audio.
        # Nota: algunos navegadores/iframes de Streamlit bloquean autoplay programático;
        # el modo externo /app/static/player.html es el fallback fiable.
        player_html = f"""
<!doctype html>
<html lang="es">
<head>
...
<style>
  ...
  .rrp-player {{ border:1px solid rgba(103,232,249,.22); border-radius:16px; padding:14px; background:linear-gradient(135deg, rgba(2,6,23,.98), rgba(15,23,42,.92)); box-shadow: inset 0 0 0 1px rgba(255,255,255,.03); }}
  ...
  audio {{ width:100%; margin:8px 0 10px 0; accent-color:#22d3ee; }}
  .controls {{ display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px; margin:8px 0; }}
  button, select {{ background:#0f172a; color:#e5e7eb; border:1px solid rgba(103,232,249,.35); border-radius:12px; padding:10px 12px; font-weight:800; cursor:pointer; }}
  ...
  .diag a {{ color:#67e8f9; text-decoration:none; }}
</style>
</head>
<body>
  <div class="rrp-player">
    <div class="top">
      <div class="title" id="nowTitle">Cargando Radar FM...</div>
      <div class="badge"><span id="count"></span> tracks</div>
    </div>
    <audio id="audio" controls preload="metadata" controlsList="nodownload"></audio>
    <div class="controls">
    ...
    <div class="status" id="status">Pulsa Play una vez. Después, Siguiente y fin de canción deberían continuar solos.</div>
    <div class="small diag">Diagnóstico: <a id="staticLink" href="#" target="_blank">abrir archivo actual</a></div>
  </div>
<script>
(() => {{
  const tracks = {tracks_json};
  const KEY_IDX = "rrp_fm_v74_idx";
  const KEY_RANDOM = "rrp_fm_v74_random";
  ...
  function save() {{ localStorage.setItem(KEY_IDX, String(idx)); localStorage.setItem(KEY_RANDOM, randomMode ? "1" : "0"); }}
  function updateRandom() {{ randomBtn.textContent = "🎲 Aleatorio: " + (randomMode ? "ON" : "OFF"); }}
  function loadTrack(keepTime=false) {{
    const t = tracks[idx];
    const oldTime = keepTime ? audio.currentTime : 0;
    audio.src = t.url;
    audio.currentTime = oldTime || 0;
    nowTitle.textContent = "Ahora: " + t.title;
    picker.value = String(idx);
    staticLink.href = t.url;
    save();
  }}
  async function tryPlay(reason) {{
    ...
    try {{
      ...
      setStatus(reason || "Reproduciendo.", "ok");
    }} catch (e) {{
      setStatus("El navegador bloqueó el autoplay. Pulsa Play una vez dentro del reproductor.", "warn");
    }}
  }}
  ...
  async function go(delta, auto=false) {{
    const wasPlaying = !audio.paused && !audio.ended;
    if (delta === 1) idx = chooseNextIndex();
    else idx = (idx - 1 + tracks.length) % tracks.length;
    loadTrack(false);
    if (auto || wasPlaying || userActivated) await tryPlay("Reproduciendo siguiente automáticamente.");
  }}

  document.getElementById("prev").addEventListener("click", () => go(-1, true));
  document.getElementById("next").addEventListener("click", () => go(1, true));
  playBtn.addEventListener("click", async () => {{
    if (audio.paused) await tryPlay("Reproduciendo.");
  ...
  picker.addEventListener("change", async () => {{ idx = parseInt(picker.value, 10) || 0; loadTrack(false); await tryPlay("Canción seleccionada."); }});
  audio.addEventListener("ended", () => go(1, true));
  audio.addEventListener("play", () => {{ userActivated = true; playBtn.textContent = "⏸️ Pausa"; }});
  audio.addEventListener("pause", () => {{ if (!audio.ended) playBtn.textContent = "▶️ Play"; }});
  audio.addEventListener("error", () => setStatus("No se pudo cargar el audio actual. Revisa que exista en static/musica.", "warn"));

  updateRandom();
  loadTrack(false);
  setStatus("Pulsa Play una vez. Luego Siguiente y fin de canción continúan solos.");
}})();
</script>
</body>
</html>
"""
        _st_components.html(player_html, height=330, scrolling=False)
        with st.expander("🔎 Diagnóstico Radar FM", expanded=False):
            st.write("Modo:", "HTML audio component v75 + external player link")
            st.write("Carpeta usada:", str(music_dir))
            st.write("Audios detectados:", len(tracks))
            st.write("Primer archivo:", tracks[0]["url"] if tracks else "")
            st.write("Rutas probadas:")
            for row_path, exists, n in debug_rows:
                st.write(f"- `{row_path}` · existe={exists} · audios={n}")
        st.markdown("</div>", unsafe_allow_html=True)
