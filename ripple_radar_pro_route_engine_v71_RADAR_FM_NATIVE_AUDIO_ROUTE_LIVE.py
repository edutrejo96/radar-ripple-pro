...
BUILD_ID = "v71_2026_05_11_RADAR_FM_NATIVE_AUDIO_ROUTE_LIVE"
BUILD_NOTE = "Radar FM con audio nativo Streamlit + fichas A→B vivas con refresh seguro"
...
def inject_music_player() -> None:
    """Radar FM robusto para Streamlit Cloud.

    v71 cambia la estrategia:
    - Ya NO depende de que el reproductor HTML/iframe pueda leer `/app/static/...`.
    - Usa `st.audio()` con bytes del MP3 seleccionado; eso es lo más fiable en Streamlit Cloud.
    - El navegador exige gesto humano para reproducir audio, así que no forzamos autoplay.
    - Mantiene selector, anterior/siguiente y aleatorio simple desde session_state.
    """
    try:
        import os as _os
            ...
            for root, _, files in _os.walk(music_dir):
                for fn in files:
                    if fn.lower().endswith(exts):
                        ...
                        title = _os.path.splitext(rel_fs.replace("\\", " / ").replace("/", " / "))[0]
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">audio nativo · static/musica</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if "rrp_fm_idx_v71" not in st.session_state:
            st.session_state["rrp_fm_idx_v71"] = 0
        if "rrp_fm_random_v71" not in st.session_state:
            st.session_state["rrp_fm_random_v71"] = False

        def _clamp_idx() -> int:
            try:
                i = int(st.session_state.get("rrp_fm_idx_v71", 0))
            except Exception:
                i = 0
            if not tracks:
                return 0
            i = max(0, min(i, len(tracks) - 1))
            st.session_state["rrp_fm_idx_v71"] = i
            return i

        cprev, cnext, crand = st.columns([1, 1, 1.25])
        with cprev:
            if st.button("⏮️ Anterior", key="rrp_fm_prev_v71"):
                st.session_state["rrp_fm_idx_v71"] = (_clamp_idx() - 1) % len(tracks)
                st.rerun()
        with cnext:
            if st.button("⏭️ Siguiente", key="rrp_fm_next_v71"):
                if st.session_state.get("rrp_fm_random_v71") and len(tracks) > 1:
                    import random as _random
                    old = _clamp_idx()
                    choices = [i for i in range(len(tracks)) if i != old]
                    st.session_state["rrp_fm_idx_v71"] = _random.choice(choices or [old])
                else:
                    st.session_state["rrp_fm_idx_v71"] = (_clamp_idx() + 1) % len(tracks)
                st.rerun()
        with crand:
            if st.button("🎲 Aleatorio: " + ("ON" if st.session_state.get("rrp_fm_random_v71") else "OFF"), key="rrp_fm_rand_v71"):
                st.session_state["rrp_fm_random_v71"] = not bool(st.session_state.get("rrp_fm_random_v71"))
                st.rerun()

        titles = [t["title"] for t in tracks]
        idx = _clamp_idx()
        picked_title = st.selectbox(
            f"{len(tracks)} canciones detectadas",
            titles,
            index=idx,
            key="rrp_fm_select_v71",
        )
        new_idx = titles.index(picked_title)
        if new_idx != idx:
            st.session_state["rrp_fm_idx_v71"] = new_idx
            idx = new_idx

        track = tracks[idx]
        st.caption("Ahora: " + track["title"])
        try:
            with open(track["path"], "rb") as fh:
                audio_bytes = fh.read()
            st.audio(audio_bytes, format=track.get("mime", "audio/mpeg"))
            st.caption("Si cambias de canción, pulsa play otra vez: el navegador bloquea el autoplay por seguridad.")
            # Enlace de diagnóstico para comprobar static serving, sin depender de él para reproducir.
            st.markdown(
                f"<a href='{html.escape(track['url'])}' target='_blank' style='font-size:12px;color:#67E8F9;'>Abrir archivo estático de diagnóstico</a>",
                unsafe_allow_html=True,
            )
        except Exception as exc:
            st.warning(f"La canción está listada, pero no se pudo leer desde el servidor: {exc}")
        st.markdown("</div>", unsafe_allow_html=True)
    except Exception as exc:
