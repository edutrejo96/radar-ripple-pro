...
BUILD_ID = "v69_2026_05_11_DEDUCTIVE_ENGINE_PROOFS"
BUILD_NOTE = "Build único: fichas A→B con pruebas fijas verificables · expander robusto · Route A→B fijo"
...
2. Click **Create Key**
3. Copia la key completa (`sk-ant-...`)
...
def inject_music_player() -> None:
    """Radar FM local con cola normal y modo aleatorio continuo.

    Versión avanzada: en vez de depender de `st.audio` o de recargar Streamlit
    al terminar una canción, levanta un servidor HTTP local de solo lectura para
    la carpeta `musica` y deja que el reproductor HTML cambie de pista en el
    navegador. Así funcionan:
    - ⏭️ siguiente sin tener que volver a pulsar play,
    - 🎲 aleatorio continuo,
    - auto-siguiente al terminar la canción,
    - sin insertar audios enormes en base64.
    """
    try:
        import os as _os
        ...
        from http.server import SimpleHTTPRequestHandler as _SimpleHTTPRequestHandler
        from http.server import ThreadingHTTPServer as _ThreadingHTTPServer
        from urllib.parse import quote as _quote

        base_dir = _os.path.abspath(_os.getcwd())
        music_dir = _os.path.join(base_dir, "musica")
        exts = (".mp3", ".wav", ".ogg", ".m4a", ".aac", ".flac")

        tracks = []
        if _os.path.isdir(music_dir):
            for root, _, files in _os.walk(music_dir):
                for fn in files:
                    if fn.lower().endswith(exts):
                        path = _os.path.abspath(_os.path.join(root, fn))
                        rel_fs = _os.path.relpath(path, music_dir)
                        rel_title = rel_fs.replace("\\", " / ").replace("/", " / ")
                        title = _os.path.splitext(rel_title)[0]
                        rel_url = "/" + "/".join(_quote(part) for part in rel_fs.replace("\\", "/").split("/"))
                        tracks.append({"title": title, "rel_url": rel_url})

        tracks = sorted(tracks, key=lambda x: x["title"].lower())

        def _find_free_port(start_port: int = 18765, max_tries: int = 80) -> int:
            for port in range(start_port, start_port + max_tries):
                with _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM) as s:
                    s.settimeout(0.15)
                    if s.connect_ex(("127.0.0.1", port)) != 0:
                        return port
            raise RuntimeError("No encontré un puerto local libre para Radar FM.")

        def _ensure_music_server() -> int:
            current = st.session_state.get("rrp_music_http_port")
            alive = bool(st.session_state.get("rrp_music_http_alive", False))
            if current and alive:
                return int(current)

            port = _find_free_port()

            class _QuietMusicHandler(_SimpleHTTPRequestHandler):
            ...
            handler = _functools.partial(_QuietMusicHandler, directory=music_dir)
            httpd = _ThreadingHTTPServer(("127.0.0.1", port), handler)
            thread = _threading.Thread(target=httpd.serve_forever, daemon=True)
            thread.start()
            st.session_state["rrp_music_http_port"] = int(port)
            st.session_state["rrp_music_http_alive"] = True
            st.session_state["rrp_music_httpd_ref"] = httpd
            return int(port)
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">música local</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if not tracks:
            st.info("No hay canciones en la carpeta `musica`. Añade MP3/WAV/OGG/M4A y reinicia Streamlit.")
            st.markdown("</div>", unsafe_allow_html=True)
            return

        port = _ensure_music_server()
        playlist = []
        for t in tracks:
            playlist.append({
                "title": t["title"],
                "url": f"http://127.0.0.1:{port}{t['rel_url']}",
            })

        playlist_json = _json.dumps(playlist, ensure_ascii=False)
        count = len(playlist)
        player_html = f"""
        <div id="rrp-fm-wrap" style="width:100%;font-family:Inter,system-ui,Segoe UI,sans-serif;color:#E2E8F0;margin-top:.55rem;">
          <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;flex-wrap:wrap;">
            <button id="rrpPrev" style="border:1px solid rgba(148,163,184,.35);background:rgba(15,23,42,.9);color:#E2E8F0;border-radius:999px;padding:6px 11px;cursor:pointer;">⏮️ Anterior</button>
            <button id="rrpNext" style="border:1px solid rgba(34,211,238,.45);background:rgba(8,47,73,.92);color:#E0F2FE;border-radius:999px;padding:6px 11px;cursor:pointer;">⏭️ Siguiente</button>
            <button id="rrpRandom" style="border:1px solid rgba(250,204,21,.45);background:rgba(71,52,0,.72);color:#FEF9C3;border-radius:999px;padding:6px 11px;cursor:pointer;">🎲 Aleatorio: OFF</button>
            <span style="font-size:12px;color:#94A3B8;">{count} canciones detectadas</span>
          </div>
          <select id="rrpSelect" style="width:100%;background:#020617;color:#E2E8F0;border:1px solid rgba(148,163,184,.32);border-radius:10px;padding:8px;margin-bottom:8px;"></select>
          <audio id="rrpAudio" controls preload="auto" style="width:100%;"></audio>
          <div id="rrpNow" style="font-size:12px;color:#94A3B8;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
        </div>
        <script>
        (function() {{
          const playlist = {playlist_json};
          const audio = document.getElementById('rrpAudio');
          const select = document.getElementById('rrpSelect');
          const now = document.getElementById('rrpNow');
          const btnPrev = document.getElementById('rrpPrev');
          const btnNext = document.getElementById('rrpNext');
          const btnRandom = document.getElementById('rrpRandom');
          const KEY_IDX = 'rrpRadarFmIndex';
          const KEY_RANDOM = 'rrpRadarFmRandomMode';
          const KEY_PLAYING = 'rrpRadarFmPlaying';
          function keyTime(i) {{ return 'rrpRadarFmTime_' + String(i); }}
          ...
          function save() {{
            ...
            localStorage.setItem(KEY_RANDOM, randomMode ? '1' : '0');
            try {{
              if (Number.isFinite(audio.currentTime)) localStorage.setItem(keyTime(idx), String(audio.currentTime));
              localStorage.setItem(KEY_PLAYING, audio.paused ? '0' : '1');
            }} catch(e) {{}}
          }}
          function paintRandom() {{
            btnRandom.textContent = randomMode ? '🎲 Aleatorio: ON' : '🎲 Aleatorio: OFF';
            btnRandom.style.background = randomMode ? 'rgba(113,63,18,.95)' : 'rgba(71,52,0,.72)';
            btnRandom.style.borderColor = randomMode ? 'rgba(251,191,36,.9)' : 'rgba(250,204,21,.45)';
          }}
          function fillSelect() {{
            select.innerHTML = '';
          ...
          function loadTrack(i, shouldPlay) {{
            idx = ((i % playlist.length) + playlist.length) % playlist.length;
            const t = playlist[idx];
            select.value = String(idx);
            now.textContent = (randomMode ? '🎲 Aleatorio continuo · ' : '▶ Cola normal · ') + t.title;
            const savedTime = parseFloat(localStorage.getItem(keyTime(idx)) || '0');
            const restoreTime = () => {{
              try {{
                if (Number.isFinite(savedTime) && savedTime > 1 && savedTime < Math.max(2, audio.duration - 2)) {{
            ...
            if (shouldPlay) {{
              setTimeout(() => audio.play().catch(() => {{
                now.textContent = 'Pulsa play una vez para desbloquear el audio del navegador · ' + t.title;
              }}), 160);
            }}
          }}
          function nextTrack(shouldPlay) {{
            const n = randomMode ? chooseRandom(idx) : idx + 1;
            loadTrack(n, shouldPlay);
          }}
          function prevTrack(shouldPlay) {{
            loadTrack(idx - 1, shouldPlay);
          }}

          fillSelect();
          paintRandom();
          loadTrack(idx, localStorage.getItem(KEY_PLAYING) === '1');
          ...
          btnRandom.addEventListener('click', () => {{
            ...
            paintRandom();
            save();
            // Al activar aleatorio, salta ya a otra canción y continúa solo en modo random.
            if (randomMode) nextTrack(true);
          }});
          select.addEventListener('change', () => {{
            idx = parseInt(select.value || '0', 10);
            loadTrack(idx, false);
          }});
          ...
          audio.addEventListener('ended', () => {{ localStorage.removeItem(keyTime(idx)); nextTrack(true); }});
        }})();
        </script>
        """
        _st_components.html(player_html, height=166)
        st.markdown("</div>", unsafe_allow_html=True)
    except Exception as exc:
        st.warning(f"No se pudo cargar el reproductor local: {exc}")
    return
...
""")
        st.code('ANTHROPIC_API_KEY=sk-ant-TU_CLAVE', language="bash")
        st.caption("Crea el archivo `.env` en la misma carpeta que el script.")

        st.markdown("**② Streamlit Cloud — Settings → Secrets**")
        st.code('ANTHROPIC_API_KEY = "sk-ant-TU_CLAVE"', language="toml")
