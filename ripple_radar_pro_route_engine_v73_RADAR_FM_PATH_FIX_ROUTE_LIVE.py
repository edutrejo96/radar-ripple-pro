...
BUILD_ID = "v73_2026_05_11_RADAR_FM_PATH_FIX_ROUTE_LIVE"
BUILD_NOTE = "Radar FM con búsqueda robusta de static/musica + fichas A→B vivas"
...
def inject_music_player() -> None:
    """Radar FM robusto para Streamlit Cloud y local.

    v73 corrige el fallo típico de despliegue: en Cloud el directorio actual puede no ser
    el mismo que el directorio del archivo Python. Por eso busca música en varias rutas:
    - carpeta del archivo actual/static/musica
    - os.getcwd()/static/musica
    - parents cercanos del archivo/static/musica

    Además muestra diagnóstico expandible para saber exactamente qué carpeta está leyendo.
    """
    try:
        import os as _os
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(34,211,238,.32);border-radius:18px;
              ...
              <div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;">
                ...
                <div style="font-size:.72rem;color:#94A3B8;border:1px solid rgba(148,163,184,.28);border-radius:999px;padding:.12rem .55rem;">audio nativo · static/musica · v73</div>
              </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if "rrp_fm_idx_v73" not in st.session_state:
            st.session_state["rrp_fm_idx_v73"] = 0
        if "rrp_fm_random_v73" not in st.session_state:
            st.session_state["rrp_fm_random_v73"] = False

        def _clamp_idx() -> int:
            try:
                i = int(st.session_state.get("rrp_fm_idx_v73", 0))
            except Exception:
                i = 0
            i = max(0, min(i, len(tracks) - 1))
            st.session_state["rrp_fm_idx_v73"] = i
            return i

        cprev, cnext, crand = st.columns([1, 1, 1.25])
        with cprev:
            if st.button("⏮️ Anterior", key="rrp_fm_prev_v73"):
                st.session_state["rrp_fm_idx_v73"] = (_clamp_idx() - 1) % len(tracks)
                st.rerun()
        with cnext:
            if st.button("⏭️ Siguiente", key="rrp_fm_next_v73"):
                if st.session_state.get("rrp_fm_random_v73") and len(tracks) > 1:
                    import random as _random
                    old = _clamp_idx()
                    choices = [i for i in range(len(tracks)) if i != old]
                    st.session_state["rrp_fm_idx_v73"] = _random.choice(choices or [old])
                else:
                    st.session_state["rrp_fm_idx_v73"] = (_clamp_idx() + 1) % len(tracks)
                st.rerun()
        with crand:
            if st.button("🎲 Aleatorio: " + ("ON" if st.session_state.get("rrp_fm_random_v73") else "OFF"), key="rrp_fm_rand_v73"):
                st.session_state["rrp_fm_random_v73"] = not bool(st.session_state.get("rrp_fm_random_v73"))
                st.rerun()
        ...
        picked_title = st.selectbox(
            ...
            titles,
            index=idx,
            key="rrp_fm_select_v73",
        )
        new_idx = titles.index(picked_title)
        if new_idx != idx:
            st.session_state["rrp_fm_idx_v73"] = new_idx
            idx = new_idx
        ...
        try:
            ...
            st.caption("Pulsa ▶️ en el control de audio. El navegador no permite autoplay sin gesto humano.")
            st.markdown(
            ...
            with st.expander("🔎 Diagnóstico Radar FM", expanded=False):
                ...
                st.write("Archivo usado:", track["path"])
